...
WINNER_MODEL_DIR = MODEL_DIR / "WINNER_MODELS"
...
class PredictionService:
    ...
    def _load_feature_names(self):
        ...
        try:
            ...
            if base_lgbm_model and hasattr(base_lgbm_model, 'booster_'):
                ...
                # 🔑 FIX: If model loaded but names couldn't be extracted, generate a dummy 
                # enriched frame to grab the full set of final columns to maintain order/completeness.
                logger.warning(f"Winner model features failed to load. Falling back to dummy enriched feature set for {self.sport}.")
                # Generate a temporary enriched dataframe to grab all columns
                dummy_state = {'sport': self.sport, 'home_team': 'H', 'away_team': 'A', 'date': '2024-01-01', 'ou_line': 50.0}
                # Use a zeroed out contextual_data dictionary to prevent errors
    ...
    def _prepare_winner_features(self, game_state_dict: Dict, contextual_data: Dict) -> Optional[pd.DataFrame]:
        ...
        try:
            # 🔑 CRITICAL FIX: Ensure the final dataframe has all the required columns in the correct order
            ...
            # Fallback: create the DataFrame with missing columns filled with 0.0
    ...
    def calculate_ml_prediction_score(self, game_state_dict: Dict, ml_weights: Dict = None) -> Optional[float]:
        ...
        # Deviation is relative to the O/U line, scaled by probability difference from 50%
    ...
    def _get_ml_winner_prob(self, X_live_final: pd.DataFrame) -> float:
        ...
        try:
            # Predict probabilities for class 1 (Home Win)
    ...
    # 🔑 --- START: REBUILT FUNCTION WITH NHL FIX ---
    def calculate_winner_prediction(self, game_state_dict: Dict) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            sport = game_state_dict.get('sport', 'NBA') # Get the sport for the fix
        ...
        # 🔑 NHL FIX: Adjust impact factor based on sport.
        # Hockey (NHL) is low-scoring, so each goal has a much higher impact on win probability.
        # NBA/NFL/MLB: 0.05 per point/run/touchdown difference is a good starting point.
        if sport == 'NHL':
            # Use 0.18: 1-goal lead -> 68%, 2-goal lead -> 86%
            score_impact_factor = 0.18 
        elif sport == 'MLB':
            # Use 0.10: 1-run lead -> 60%, 2-run lead -> 70%
            score_impact_factor = 0.10
        else:
            # Default for high-scoring sports (NBA, NFL, etc.)
            score_impact_factor = 0.05 
        
        dynamic_home_prob = np.clip(0.5 + (score_diff * score_impact_factor), 0.05, 0.95)
        
        # Blend: Weight ML prob by *remaining* time, dynamic prob by *elapsed* time
    ...
    # 🔑 --- END: REBUILT FUNCTION WITH NHL FIX ---
    ...
    def get_shap_plot_data(self, game_state_dict: Dict) -> Dict[str, List[float]]:
        ...
        feature_values = []
        # FIX: Pass the full state dict for robust feature retrieval
        ...
        for feature_key_human in features:
            # Simple heuristic to match human-readable names to snake_case features
            key_snake_case = feature_key_human.lower().replace(' ', '_').replace('.', '').replace('/', '_').replace(':', '')
            val = raw_features.get(key_snake_case)
            
            # CRITICAL CHECK: Look for derived features that aren't in the raw input dict
            if val is None:
                ...
                # Fallback to 0.0 if not found/derivable
                else:
                    val = 0.0
    ...
    def calculate_shap_values(self, game_state_dict: Dict, ml_weights: Dict = None) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            # Note: We pass the feature vector for class 1 (Over)
            ...
            for feature, value in zip(self.feature_names, shap_values_class1[0]):
                # Scaling the SHAP values for better visualization impact
    ...
    def calculate_over_under_probability(self, current_score, over_under, status, period=None, time_remaining=None, contextual_data=None, ml_weights: Dict = None):
        ...
        # FIX: Ensure over_under is always treated as a float when passed to the simulator
        ...
        # CRITICAL FIX: If ML score prediction failed (e.g., O/U line was missing/zero), 
        # it's safer to pass None to the simulator, and let the simulator use internal fallbacks.
